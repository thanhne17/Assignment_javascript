...
const EditCate = {
    async print(id){
        const { data } = await axios.get(`http://localhost:3001/cate_pr/${id}`);
        return /* html */ `
                ${NavDashboad.print()}
                      ...
                      <div class="shadow overflow-hidden sm:rounded-md">
                        <div class="px-4 py-5 bg-white sm:p-6">
                          <div class="grid grid-cols-6 gap-6">
                            ...
                            <input value="${data.id}" type="text" name="id" id="id" autocomplete="given-name">

                          <div class="col-span-6 sm:col-span-3">
                            <label for="last-name" class="block text-sm font-medium text-gray-700">Tên danh mục</label>
                            <input value="${data.title}" type="text" name="name" id="name" autocomplete="family-name" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                          </div>
